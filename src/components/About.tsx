...
import { Cloud, Bot, Activity, ArrowUpRight } from "lucide-react";
          ...
          {expertise.map((item, index) => {
            ...
            return (
              <Reveal key={item.title} delay={(index % 3) * 70} y={18}>
                <a
                  href="#projects"
                  className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border p-6 transition-all duration-500 hover:-translate-y-1 hover:border-foreground hover:bg-foreground"
                >
                  {/* Ghost index number */}
                  ...
                  <h3 className="relative mt-5 flex items-center gap-2 text-lg font-semibold tracking-tight transition-colors duration-500 group-hover:text-background">
                    {item.title}
